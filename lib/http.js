...
export async function addCborldRoutes({app, service} = {}) {
  ...
  const routes = {
    cborldRegistryEntries:
      `${routePrefix}/:localId${cfg.routes.cborldRegistryEntries}`
  };
  routes.cborldRegistryEntries =
    `${routes.cborldRegistryEntries}/:registryEntryId`;
  ...
  // store a new CBOR-LD registry entry
  app.options(routes.cborldRegistryEntries, cors());
  app.post(
    routes.cborldRegistryEntries,
    cors(),
    validate({bodySchema: createCborLdRegistryEntryBody}),
    getConfigMiddleware,
    ...
    asyncHandler(async (req, res) => {
      ...
      const {id, registryEntry} = req.body;
      const content = {id, registryEntry};
      const meta = {type: 'CborLdRegistryEntry'};
      ...
      try {
        ...
        if(e.name === 'AbortError') {
          throw new BedrockError('Duplicate CBOR-LD registry entry.', {
            name: 'DuplicateError',
            details: {
              httpStatusCode: 409,
              public: true
      ...
      const location = `${config.id}/${cfg.routes.cborldRegistryEntries}/` +
        encodeURIComponent(id);
      res.status(201).location(location).json({id, registryEntry, sequence: 0});

      // meter operation usage
      metering.reportOperationUsage({req});
    }));

  // update an existing CBOR-LD registry entry
  app.options(routes.cborldRegistryEntries, cors());
  app.post(
    routes.cborldRegistryEntries,
    cors(),
    validate({bodySchema: updateCborLdRegistryEntryBody}),
    getConfigMiddleware,
    middleware.authorizeServiceObjectRequest(),
    asyncHandler(async (req, res) => {
      const {config} = req.serviceObject;
      ...
      const {id, registryEntry, sequence} = req.body;
      const content = {id, registryEntry};
      const meta = {type: 'CborLdRegistryEntry'};
      ...
      try {
          ...
          async mutator({doc}) {
            if(doc.meta.type !== 'CborLdRegistryEntry') {
              ...
              const error = new Error(
                'Existing document is not a CBOR-LD registry entry.');
        ...
        if(e.name === 'AbortError') {
          throw new BedrockError(
            'Could not update CBOR-LD registry entry; unexpected sequence.',
            'InvalidStateError', {
              expected: sequence - 1,
              actual: e.actual,
              httpStatusCode: 409,
      ...
      res.json({id, registryEntry, sequence});

      // meter operation usage
      metering.reportOperationUsage({req});
    }));

  // get a stored CBOR-LD registry entry
  app.get(
    routes.cborldRegistryEntries,
    cors(),
    getConfigMiddleware,
    ...
    asyncHandler(async (req, res) => {
      ...
      let doc;
      try {
        doc = await documentStore.get({id: req.params.registryEntryId});
      } catch(e) {
        // document not found
        if(e.name === 'NotFoundError') {
          throw new BedrockError('CBOR-LD registry entry not found.', {
            name: 'NotFoundError',
            details: {
              httpStatusCode: 404,
              public: true
      ...
      if(doc.meta.type !== 'CborLdRegistryEntry') {
        // invalid meta type, treat as registry entry not found
        throw new BedrockError('CBOR-LD registry entry not found.', {
          name: 'NotFoundError',
          details: {
            httpStatusCode: 404,
      ...
      const result = {
        id: doc.content.id,
        registryEntry: doc.content.registryEntry,
  ...
  app.post(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        if(e.name === 'AbortError') {
          throw new BedrockError('Duplicate context.', {
            name: 'DuplicateError',
            details: {
              httpStatusCode: 409,
              public: true
  ...
  app.get(
    ...
    asyncHandler(async (req, res) => {
      ...
      try {
        ...
        // document not found
        if(e.name === 'NotFoundError') {
          throw new BedrockError('Context not found.', {
            name: 'NotFoundError',
            details: {
              httpStatusCode: 404,
              public: true
